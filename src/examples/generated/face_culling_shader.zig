...
//    Overview:
//    =========
//    Shader program: 'face_culling':
//        Get shader desc: shd.faceCullingShaderDesc(sg.queryBackend());
//        Vertex Shader: vs
//        Fragment Shader: fs
//        Attributes:
//            ATTR_face_culling_position => 0
//            ATTR_face_culling_color0 => 1
//    Bindings:
//        Uniform block 'vs_params':
//            Zig struct: VsParams
//            Bind slot: UB_vs_params => 0
//
pub const ATTR_face_culling_position = 0;
pub const ATTR_face_culling_color0 = 1;
...
const vs_source_glsl410 = [295]u8 {
    ...
    0x33,0x20,0x63,0x6f,0x6c,0x6f,0x72,0x30,0x3b,0x0a,0x0a,0x76,0x6f,0x69,0x64,0x20,
...
const vs_source_glsl300es = [277]u8 {
    ...
    0x6e,0x3b,0x0a,0x6f,0x75,0x74,0x20,0x76,0x65,0x63,0x33,0x20,0x63,0x6f,0x6c,0x6f,
...
const vs_source_metal_macos = [522]u8 {
    ...
    0x33,0x20,0x63,0x6f,0x6c,0x6f,0x72,0x30,0x20,0x5b,0x5b,0x61,0x74,0x74,0x72,0x69,
...
//
//    @vertex
//    fn main(@location(0) position_1_param : vec3f, @location(1) color0_param : vec3f) -> main_out {
...
const vs_source_wgsl = [725]u8 {
    ...
    0x7a,0x2c,0x20,0x31,0x2e,0x30,0x66,0x29,0x29,0x3b,0x0a,0x20,0x20,0x63,0x6f,0x6c,
    0x6f,0x72,0x20,0x3d,0x20,0x63,0x6f,0x6c,0x6f,0x72,0x30,0x3b,0x0a,0x20,0x20,0x72,
...
pub fn faceCullingShaderDesc(backend: sg.Backend) sg.ShaderDesc {
    var desc: sg.ShaderDesc = .{};
    desc.label = "face_culling_shader";
    switch (backend) {
        .GLCORE => {
...
pub fn faceCullingAttrSlot(attr_name: []const u8) ?usize {
...
pub fn faceCullingTextureSlot(tex_name: []const u8) ?usize {
    _ = tex_name;
    return null;
}
pub fn faceCullingSamplerSlot(smp_name: []const u8) ?usize {
    _ = smp_name;
    return null;
}
pub fn faceCullingUniformBlockSlot(ub_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        return 0;
    }
    return null;
}
pub fn faceCullingUniformBlockSize(ub_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        return @sizeOf(VsParams);
    }
    return null;
}
pub fn faceCullingUniformOffset(ub_name: []const u8, u_name: []const u8) ?usize {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
...
pub fn faceCullingUniformDesc(ub_name: []const u8, u_name: []const u8) ?sg.GlslShaderUniform {
    if (std.mem.eql(u8, ub_name, "vs_params")) {
        if (std.mem.eql(u8, u_name, "mvp")) {
...
pub fn faceCullingStorageBufferSlot(sbuf_name: []const u8) ?usize {
    _ = sbuf_name;
    return null;
}
pub fn faceCullingStorageImageSlot(simg_name: []const u8) ?usize {
    _ = simg_name;
    return null;
}
