...
// A long, rotated rectangle makes stair-stepped pixel coverage easy to see.
...
export fn init() void {
    ...
    state.single_pipeline = sg.makePipeline(.{
        .shader = scene_shader,
        .layout = scene_layout,
        // This 2D coverage demonstration has no depth attachment.
...
fn recreateTargets(width: i32, height: i32) void {
    ...
    // Mode 1: one ordinary image serves as both attachment and sampled texture.
    ...
    // Mode 2: rasterization writes four coverage samples per pixel here.
    ...
    // A multisampled image cannot be sampled like a normal texture. At the end
    // of the pass Sokol averages/resolves it into this one-sample image.
...
export fn frame() void {
    ...
    sg.endPass(); // mode 2 performs its MSAA resolve here
...
pub fn main() void {
    sapp.run(.{
        ...
        .width = 900,
        .height = 600,
        .sample_count = 1, // presentation is single-sample; mode 2 resolves first
