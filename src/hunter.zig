...
test "vision cone is frontal" {
    ...
    try std.testing.expect(inVisionCone(0, .{ .x = 0, .y = 0, .z = 5 }, half));
    try std.testing.expect(inVisionCone(0, .{ .x = 3, .y = 0, .z = 5 }, half));
    try std.testing.expect(!inVisionCone(0, .{ .x = 0, .y = 0, .z = -5 }, half));
    try std.testing.expect(!inVisionCone(0, .{ .x = 5, .y = 0, .z = -1 }, half));
    try std.testing.expect(inVisionCone(std.math.pi, .{ .x = 0, .y = 0, .z = -5 }, half));
}
